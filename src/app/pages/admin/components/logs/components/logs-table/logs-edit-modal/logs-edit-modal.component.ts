...
@Component({
  ...
  styleUrls: [('./logs-edit-modal.component.scss')],
  templateUrl: './logs-edit-modal.component.html'
})

export class LogsEditModalComponent implements OnInit {
  ...
  onSubmit(values: LogsInterface): void {
    this.submitted = true;
    if (this.form.valid) {
      this.service
        .editLogs(values)
  ...
  private showToast(data: any, values: LogsInterface) {
